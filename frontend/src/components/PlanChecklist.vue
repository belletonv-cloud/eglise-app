  ...
  <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4">
    ...
    <div class="space-y-1">
      <div v-for="item in items" :key="item.id"
        ...
        <div class="flex-1">
          <span :class="item.done ? 'line-through text-gray-400' : 'text-gray-700 dark:text-gray-300'"
            class="text-sm">{{ item.label }}</span>
          <span v-if="item.position" class="text-xs text-gray-400 ml-2">[{{ item.position }}]</span>
        </div>
        <button @click="remove(item)" class="text-red-400 hover:text-red-600 text-xs opacity-0 group-hover:opacity-100 cursor-pointer">✕</button>
      </div>
    </div>
