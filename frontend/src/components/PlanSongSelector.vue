<template>
  <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" @click.self="$emit('close')">
    <div class="bg-white rounded-xl p-6 w-full max-w-xl shadow-xl max-h-[80vh] flex flex-col">
      <h3 class="text-xl font-bold mb-4">Ajouter un chant</h3>

      <input v-model="search" placeholder="Rechercher un chant..."
      ...
      <div v-else class="flex-1 overflow-y-auto space-y-2">
        <div v-for="song in filteredSongs" :key="song.id"
          ...
          :class="{ 'border-blue-400 bg-blue-50': selected?.id === song.id }">
          <div class="font-medium text-gray-800">{{ song.title }}</div>
          <div v-if="song.author" class="text-sm text-gray-500">{{ song.author }}</div>
    <div v-if="selected?.id === song.id && song.arrangements?.length" class="mt-2">
      <label class="text-sm font-medium text-gray-600">Arrangement</label>
      <select v-model="selectedArrangementId"
        class="w-full border border-gray-300 rounded-lg px-2 py-1 mt-1 text-sm">
        <option :value="null">Choisir...</option>
        <option v-for="a in song.arrangements" :key="a.id" :value="a.id">
          {{ a.name }} {{ a.key ? '(' + a.key + ')' : '' }}
        </option>
      </select>
      <div v-if="selectedArrangementId && !selectedArrangementHasChart" class="mt-2 p-2 bg-yellow-50 border border-yellow-200 text-yellow-800 rounded text-sm">
        ⚠️ L'arrangement sélectionné ne contient pas de grille d'accords. Tu pourras l'ajouter quand même, mais il n'y aura pas d'accords affichés dans la fiche chant.
      </div>
      <div v-if="selectedArrangementId" class="mt-1">
        <label class="text-sm font-medium text-gray-600">Transposition</label>
        <input v-model="transposedKey" placeholder="Ex: C, Dm, G..."
          class="w-full border border-gray-300 rounded-lg px-2 py-1 mt-1 text-sm">
      </div>
    </div>
        </div>
        <div v-if="filteredSongs.length === 0" class="text-center py-8 text-gray-400">
          Aucun chant trouvé
        </div>
      </div>
...
const emit = defineEmits<{ close: []; select: [songId: number, arrangementId: number, transposedKey: string | null] }>()
...
const filteredSongs = computed(() => {
  ...
  return songs.value.filter(s =>
    !q || s.title.toLowerCase().includes(q) || (s.author && s.author.toLowerCase().includes(q))
  )
})
