<template>
  <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
    <div class="mb-4 flex items-center justify-between">
      ...
      <button
        @click="showAdd = true"
        class="cursor-pointer rounded-lg bg-blue-600 px-3 py-1.5 text-sm text-white hover:bg-blue-700"
      >
      ...
      <div class="space-y-4">
        ...
        >
          <div class="mb-3 flex items-center justify-between">
            <div>
              <h4 class="text-base font-semibold text-gray-800">{{ group.name }}</h4>
              <div class="mt-1 flex items-center gap-3 text-xs">
                <span class="text-emerald-600">✅ {{ group.confirmed }}</span>
                <span class="text-red-600">❌ {{ group.declined }}</span>
                <span class="text-amber-600">⚪ {{ group.pending }}</span>
              </div>
            </div>
          </div>
          ...
          <table class="min-w-full">
            ...
            <tbody class="divide-y divide-gray-100">
              <tr v-for="p in group.people" :key="p.id" class="hover:bg-gray-50">
                <td class="px-2 py-2 text-sm text-gray-700">{{ p.position || '—' }}</td>
                <td class="px-2 py-2 text-sm font-medium text-gray-800">{{ p.first_name }} {{ p.last_name }}</td>
                <td class="px-2 py-2 text-sm">
                  <span :class="statusClass(p.status)" class="rounded-full px-2 py-0.5 text-xs font-medium">
                    {{ p.status || 'pending' }}
                  </span>
                </td>
                <td class="px-2 py-2 text-right">
                  <button @click="updateStatus(p, 'confirmed')" class="mr-2 text-xs text-emerald-600 hover:underline">✓</button>
                  <button @click="updateStatus(p, 'pending')" class="mr-2 text-xs text-amber-600 hover:underline">○</button>
                  <button @click="updateStatus(p, 'declined')" class="mr-2 text-xs text-red-600 hover:underline">✕</button>
                  <button @click="remove(p)" class="text-xs text-gray-500 hover:underline">{{ $t('schedulePeople.remove') }}</button>
                </td>
              </tr>
            </tbody>
          </table>
    ...
    <div v-if="showAdd" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="showAdd = false">
      <div class="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
        <h4 class="mb-4 text-lg font-bold">{{ $t('schedulePeople.add_person.title') }}</h4>
        <form @submit.prevent="addPerson" class="space-y-4">
          <div>
            <label class="mb-1 block text-sm font-medium text-gray-700">{{ $t('schedulePeople.add_person.member') }}</label>
            <select v-model="newPerson.member_id" required class="w-full rounded-lg border border-gray-300 px-3 py-2">
          ...
          <div>
            <label class="mb-1 block text-sm font-medium text-gray-700">{{ $t('schedulePeople.add_person.role') }}</label>
            <input
              v-model="newPerson.position"
              :placeholder="$t('schedulePeople.add_person.role_placeholder')"
              class="w-full rounded-lg border border-gray-300 px-3 py-2"
            />
          </div>
          <div>
            <label class="mb-1 block text-sm font-medium text-gray-700">{{ $t('schedulePeople.add_person.team') }}</label>
            <select v-model="newPerson.team_id" class="w-full rounded-lg border border-gray-300 px-3 py-2">
          ...
          <div class="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" class="cursor-pointer rounded-lg bg-blue-600 px-4 py-2 text-white hover:bg-blue-700">
              {{ $t('schedulePeople.add_person.add') }}
            </button>
          </div>
        </form>
...
const groupedTeams = computed(() => {
  ...
  return Array.from(map.entries()).map(([name, teamPeople]) => ({
    ...
    pending: teamPeople.filter((p) => !p.status || p.status === 'pending').length,
  }))
})
...
async function setAvailability(status: 'confirmed' | 'declined') {
  ...
  const mine = people.value.find((p) => p.member_id === currentMemberId.value)
  if (!mine) return
