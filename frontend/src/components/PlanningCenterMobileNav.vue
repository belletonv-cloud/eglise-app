<template>
  <div class="md:hidden fixed bottom-0 left-0 right-0 border-t border-gray-200 bg-white/95 backdrop-blur">
    <div class="mx-auto grid max-w-md grid-cols-5 gap-1 px-2 py-2 text-[11px] text-gray-700">
      <button
        v-for="item in items"
        :key="item.key"
      ...
      >
        <span :class="isActive(item.key) ? 'text-[#2ECC71]' : 'text-gray-700'">
          {{ item.icon }}
        </span>
        <span :class="isActive(item.key) ? 'font-semibold text-[#2ECC71]' : ''">
          {{ item.label }}
        </span>
      </button>
    </div>
