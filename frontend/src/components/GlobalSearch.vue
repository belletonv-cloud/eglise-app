<template>
  <div class="relative">
    <div class="flex items-center gap-2 px-3 py-2 bg-gray-100 dark:bg-gray-700 rounded-lg border border-transparent focus-within:border-emerald-300 dark:focus-within:border-emerald-700">
    ...
    <div
      v-if="query.trim().length >= 2 && (loading || results.length || searched)"
    ...
    >
      ...
      <div v-if="!loading && searched && results.length === 0" class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
        {{ $t('search.no_results') }}
      </div>
    </div>
  </div>
...
function matchesQuickLink(link: { title: string; description: string }, term: string): boolean {
  const haystack = `${link.title} ${link.description}`.toLowerCase()
  return haystack.includes(term)
}

function buildResults(term: string, apiResults: any[]) {
  const shortcuts = quickLinks.value.filter((link) => matchesQuickLink(link, term))
  const merged = [...shortcuts, ...apiResults]
  const seen = new Set<string>()
  return merged.filter((item) => {
    const key = item.type === 'route' ? `route:${item.id}` : `${item.type}:${item.id}`
...
const search = () => {
  ...
  timeout = setTimeout(async () => {
    ...
    try {
      ...
      const res = await authenticatedFetch(`${base}/api/search?q=${encodeURIComponent(term)}`)
      if (!res.ok) throw new Error(`HTTP ${res.status}`)
      const data = await res.json()
      if (requestId !== activeRequest) return
      results.value = buildResults(term.toLowerCase(), Array.isArray(data.results) ? data.results : [])
    } catch {
      if (requestId !== activeRequest) return
