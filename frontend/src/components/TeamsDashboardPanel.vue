  ...
  <div class="space-y-4">
      ...
      <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p class="mt-1 text-sm text-gray-600">
            Répartition des équipes façon Planning Center : Band, Audio/Visual et équipes support.
          </p>
        </div>
        <div class="flex flex-wrap gap-2 text-xs font-medium">
          <span class="rounded-full bg-white px-3 py-1 text-gray-700 ring-1 ring-gray-200">
            {{ teamCount }} teams
          </span>
          <span class="rounded-full bg-white px-3 py-1 text-gray-700 ring-1 ring-gray-200">
            {{ assignedCount }} personnes
          </span>
          <span class="rounded-full bg-white px-3 py-1 text-emerald-700 ring-1 ring-emerald-200">
    ...
    >
      <div
        v-for="n in 2"
        :key="n"
        class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm animate-pulse"
      >
        <div class="h-6 w-40 rounded bg-gray-200"></div>
        <div class="mt-3 h-4 w-56 rounded bg-gray-100"></div>
        <div class="mt-6 space-y-3">
          <div v-for="i in 4" :key="i" class="h-14 rounded-xl bg-gray-100"></div>
        </div>
      </div>
    </div>
    ...
    <div v-else class="space-y-4">
      <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        >
          <div class="flex items-start justify-between gap-4">
            <div>
              <h4 class="text-xl font-semibold text-gray-900">{{ board.title }}</h4>
              <p class="mt-1 text-sm text-gray-500">{{ board.subtitle }}</p>
            </div>
            <div class="flex items-center gap-2 text-xs font-semibold">
              <span class="rounded-full bg-emerald-50 px-2.5 py-1 text-emerald-700">
                ✓ {{ board.stats.ok }}
              </span>
              <span class="rounded-full bg-red-50 px-2.5 py-1 text-red-600">
                ✕ {{ board.stats.missing }}
              </span>
          ...
          >
            {{ board.stats.pending }} équipe(s) sans affectation complète.
          </div>

          <div
            v-if="board.roles.length === 0"
            class="mt-5 rounded-xl border border-dashed border-gray-200 px-4 py-8 text-center text-sm text-gray-500"
          >
            {{ board.emptyMessage }}
          </div>
              ...
              <div class="divide-y divide-gray-100 bg-white">
                ...
                >
                  <div class="flex min-w-0 items-center gap-3">
                    <div
                      class="flex h-9 w-9 items-center justify-center rounded-full bg-gray-200 text-xs font-bold text-gray-700"
                    >
                  ...
                  <a
                    ...
                    class="shrink-0 rounded-full p-2 text-gray-500 transition hover:bg-gray-100 hover:text-gray-900"
      ...
      >
        <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div>
            <h4 class="text-lg font-semibold text-gray-900">Support teams</h4>
            <p class="mt-1 text-sm text-gray-500">
              Accueil, enfants et autres équipes utiles autour du service.
            </p>
          </div>
        ...
        <div class="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-3">
          ...
          >
            <div class="flex items-start justify-between gap-3">
              <div>
                <h5 class="text-base font-semibold text-gray-900">{{ team.name }}</h5>
                <p class="mt-1 text-xs text-gray-500">{{ team.description || supportLabel(team) }}</p>
              </div>
              <span class="rounded-full bg-white px-2.5 py-1 text-xs font-semibold text-gray-700 ring-1 ring-gray-200">
            ...
            <div
              ...
              class="mt-4 rounded-xl border border-dashed border-gray-200 bg-white px-3 py-6 text-center text-sm text-gray-500"
            >
...
function buildStats(teams: TeamDetail[]): BoardStats {
  ...
  const ok = members.filter((member) => Boolean(member.position?.trim())).length;
...
async function load() {
  ...
  try {
    const summaries = (await api.getTeams({ page: 1, size: 100 } as any)) as
      | TeamSummary[]
      | null;
