<template>
  <div class="annotations-panel">
    <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-3">
      📝 Annotations
      <span class="text-sm font-normal text-gray-500">({{ annotations.length }})</span>
    </h3>
        ...
        <div class="flex items-start justify-between gap-2">
          <div class="flex-1">
            <div class="text-sm font-medium text-gray-700 dark:text-gray-300">
              ...
              <span v-if="ann.is_shared" class="text-xs text-blue-500 ml-2">🌍 Partagée</span>
              <span v-else class="text-xs text-gray-400 ml-2">🔒 Privée</span>
            </div>
          ...
          <div v-if="ann.member_id === currentMemberId" class="flex gap-1 shrink-0">
            <button @click="editAnnotation(ann)" class="p-1 text-gray-400 hover:text-blue-500 cursor-pointer" title="Modifier">✏️</button>
            <button @click="removeAnnotation(ann.id)" class="p-1 text-gray-400 hover:text-red-500 cursor-pointer" title="Supprimer">🗑️</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="editing" class="bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700 p-3">
      <textarea v-model="form.content" rows="3" placeholder="Ajouter une annotation..."
      ...
      <div class="flex items-center justify-between mt-2">
        ...
        <div class="flex gap-2">
          <button @click="cancelEdit" class="px-3 py-1.5 text-sm text-gray-600 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg cursor-pointer">Annuler</button>
