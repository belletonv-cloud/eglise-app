<template>
    <div class="song-detail" v-if="song">
        <div class="header">
            ...
            <p v-if="song.ccli_number" class="ccli">
                {{ t("song.ccli") }} {{ song.ccli_number }}
            </p>
        </div>

        <div class="song-info">
            <p v-if="song.themes">
                <strong>{{ t("song.themes") }}</strong> {{ song.themes }}
            </p>
            <p v-if="song.notes">
                <strong>{{ t("song.notes") }}</strong> {{ song.notes }}
            </p>
        </div>

        <div
            class="arrangements"
            v-if="song.arrangements && song.arrangements.length > 0"
        >
            ...
            >
                <table class="min-w-full text-sm">
                    <thead class="bg-gray-50 dark:bg-gray-900/50">
                        <tr>
                            <th
                                class="px-3 py-2 text-left font-semibold text-gray-500 dark:text-gray-400"
                            >
                                Arrangement
                            </th>
                            <th
                    ...
                    <tbody>
                        <tr
                            ...
                            :class="{
                                'bg-blue-50': selectedArrangementId === arr.id,
                            }"
                            @click="selectArrangement(arr.id)"
                        >
                            <td
                                class="px-3 py-2 font-medium text-gray-800 dark:text-gray-100"
                            >
                                {{ arr.name }}
                            </td>
                            <td
                            ...
                            <td
                                class="px-3 py-2 text-gray-600 dark:text-gray-300"
                            >
                                {{ arr.tempo ? `${arr.tempo} BPM` : "—" }}
                            </td>
                            <td
                                class="px-3 py-2 text-gray-600 dark:text-gray-300"
                            >
                                {{ arr.chord_chart ? "Yes" : "No" }}
                            </td>
                            <td
            ...
            <div class="arrangement-controls">
                ...
                <button
                    @click="exportPDF"
                    class="pdf-btn"
                    :disabled="!currentArrangement?.chord_chart"
                >
                    📄
                    {{
                        exportingPdf
                ...
                <span v-if="isOffline" class="offline-indicator">{{
                    t("song.available_offline")
                }}</span>
            </div>
        </div>
        <div
            v-else-if="song.arrangements && song.arrangements.length === 0"
        ...
        <div class="transpose-controls" v-if="currentArrangement">
            ...
            <div class="controls">
                ...
                <span class="semitones"
                    >{{ t("song.transpose.semitones") }} {{ semitones }}</span
                >
            </div>
            <div class="actions">
                ...
                <button @click="transposeDown">
                    {{ t("song.transpose.down") }}
                </button>
                <button @click="resetTransposition">
                    {{ t("song.transpose.reset") }}
                </button>
            </div>
        </div>

        <div
            v-if="!displayedChart && currentArrangement"
        ...
        <div class="chord-chart" v-else-if="displayedChart">
            <div class="chart-content">
                <template v-for="(line, idx) in chordLines" :key="idx">
                    ...
                    ></div>
                    <div
                        v-else-if="line.type === 'standalone'"
        ...
        <div v-if="currentArrangement" class="media-section">
            <h3>
                {{ t("song.media") }}
                <span v-if="mediaItems.length" class="count"
            ...
            <div class="media-list">
                ...
                >
                    ...
                    <span class="media-type-badge">{{
                        media.file_type || t("song.file")
                    }}</span>
                    <button
                        @click="deleteMedia(media.id)"
                        class="media-delete-btn"
                        :title="t('song.delete_media')"
                    >
                        ✕
                    </button>
                </div>
        ...
        <div
            v-if="showEditor && currentArrangement"
            class="modal-overlay"
        ...
        <div
            v-if="showMediaUpload && currentArrangement"
            class="modal-overlay"
            @click.self="showMediaUpload = false"
        >
            <MediaUpload
                :arrangement-id="currentArrangement.id"
        ...
        <div v-if="currentArrangement" class="mt-8">
            <ArrangementAnnotations
                :arrangement-id="currentArrangement.id"
                :key="'ann-' + currentArrangement.id"
            />
        </div>
    </div>
</template>
...
async function loadMedia() {
    ...
    try {
        mediaItems.value = await api.getArrangementMedia(
...
async function exportPDF() {
    ...
    try {
        ...
        doc.save(
            `${title.replace(/[^a-zA-Z0-9]/g, "_")}_${arrName.replace(/[^a-zA-Z0-9]/g, "_")}.pdf`,
        );
...
const chordLines = computed<ParsedLine[]>(() =>
    parseChordPro(displayedChart.value),
);
...
const loadArrangement = () => {
    ...
    const arr = song.value.arrangements.find(
        (a: any) => a.id === selectedArrangementId.value,
    );
    if (arr) {
...
const transpose = () => {
    semitones.value =
        getSemitonesFromC(targetKey.value) -
        getSemitonesFromC(originalKey.value);
