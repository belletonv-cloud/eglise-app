    ...
    >
        ...
        <div v-if="audio" class="space-y-2">
            <div class="flex items-center gap-2">
                <p
                    v-if="audio.audio_title"
                    class="text-sm font-medium text-gray-700 dark:text-gray-300"
                >
                    {{ audio.audio_title }}
                </p>
                <span
                    v-if="audio.audio_duration_seconds"
                    class="text-xs text-gray-400 dark:text-gray-500"
                    >⏱ {{ fmtTime(audio.audio_duration_seconds) }}</span
                >
            </div>
            <audio
                v-if="audio.audio_url"
                :src="`${apiBase}/plans/${props.planId}/audio/stream`"
                controls
                class="w-full"
            />
            <div
                v-for="att in audio.attachments"
                :key="att.id"
            ...
            >
                <div class="flex-1 min-w-0">
                    ...
                    <audio
                        ...
                        controls
                        class="w-full mt-1"
                        v-if="att.file_type === 'audio'"
                    />
                </div>
                <button
                    @click="deleteAudio(att.id)"
                    class="text-red-400 hover:text-red-600 text-xs cursor-pointer ml-2"
                >
                    {{ $t("sermonAudio.delete") }}
                </button>
            </div>
        </div>
        ...
        <div class="mt-3">
            ...
            <div v-if="processing" class="space-y-2 mb-2">
                ...
                <div
                    v-if="estimatedTime"
                    class="text-xs text-gray-400 dark:text-gray-500 pl-6"
                >
        ...
        >
            <div class="flex items-center justify-between mb-2">
                ...
                >
                    {{ $t("sermonAudio.segments") }}
                </h4>
                <div class="flex items-center gap-3">
                    <span
                        v-if="segmentsData.duration_seconds"
                        class="text-xs text-gray-400 dark:text-gray-500"
                        >⏱ {{ fmtTime(segmentsData.duration_seconds) }}</span
                    >
                    <span
                        v-if="segmentsData.songs?.length"
                        class="text-xs text-gray-400 dark:text-gray-500"
                        >{{ segmentsData.songs.length }}
                        {{ $t("sermonAudio.songs").toLowerCase() }}</span
                    >
                </div>
            </div>
            ...
            <div v-if="segmentsData.songs?.length" class="mb-3">
                <div
                    v-for="(s, i) in segmentsData.songs"
                    :key="i"
            ...
            <div class="max-h-64 overflow-y-auto space-y-1">
                <div
                    v-for="(seg, i) in segmentsData.segments"
                    :key="i"
                ...
                >
                    <span
                        ...
                        >{{ fmtTime(seg.start_seconds) }}</span
                    >
                    <span
                        :class="typeClass(seg.segment_type)"
                        class="shrink-0 w-20 text-center rounded px-1 py-0.5 font-medium"
                        >{{ typeLabel(seg.segment_type) }}</span
                    >
                    <span
                        class="flex-1 text-gray-600 dark:text-gray-400 truncate"
                        >{{ seg.title || seg.text || "" }}</span
                    >
                    <span
                        v-if="seg.confidence"
                        class="shrink-0 text-gray-400 dark:text-gray-500 w-8 text-right"
                        >{{ (seg.confidence * 100).toFixed(0) }}%</span
                    >
                </div>
                <p
                    v-if="!segmentsData.segments?.length"
                    class="text-center py-2 text-gray-400 dark:text-gray-500 text-xs"
                >
                    {{ $t("sermonAudio.no_segments") }}
                </p>
            </div>
        </div>
    </div>
...
const upload = async () => {
    ...
    try {
        ...
        const result = await api.uploadPlanAudio(
            props.planId,
            file,
            audioTitle.value || t("sermonAudio.upload_title"),
        );
