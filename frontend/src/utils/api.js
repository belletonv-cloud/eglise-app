...
const _d = (day) => `${_y}-${_m}-${String(day).padStart(2, '0')}`;
function toKebab(s) {
    return s.replace(/^[A-Z]/, c => c.toLowerCase())
        .replace(/([A-Z])/g, '-$1')
        .toLowerCase();
}
function guessRoute(prop) {
    const patterns = [
        [/^get(.+)$/, 'GET', false],
        [/^create(.+)$/, 'POST', true],
        [/^update(.+)$/, 'PUT', true],
...
function buildUrl(route, args, base) {
    let path = route.path;
...
async function tryCall(prop, args) {
...
function extractBody(args) {
    for (const a of args) {
        if (typeof a === 'object' && a !== null && !Array.isArray(a) && !(a instanceof URLSearchParams))
            return a;
    }
    return undefined;
}
const mockFallback = {
    ...
    getVolunteerPreferences: () => ({ available: true, max_services_per_month: 4, notes: '', unavailable_dates: [] }),
        ...
        get(_target, prop) {
            ...
            cache[prop] = async (...args) => {
                ...
                if (prop.startsWith('delete') || prop.startsWith('remove'))
                    return { deleted: true };
                if (prop === 'syncPCO')
                    return { synced: true, results: { imported: 5, updated: 3, errors: [] } };
                return null;
            };
            return cache[prop];
        }
