...
export function transposeLine(line, semitones) {
    return line.replace(/\[([^\]]+)\]|(?<!\w)([A-Ga-g][#b]?(?:m|min|dim|aug|sus[24]?|maj7?|7|9|11|13|add\d+|sus|dim7|aug7)?(?:\/[A-Ga-g][#b]?)?)(?!\w)/g, (match, inBrackets, alone) => {
        const chord = inBrackets || alone;
...
export function transposeChordChart(chart, fromKey, toKey) {
    ...
    if (fromIndex === -1)
        return chart; // Tonalité inconnue
...
export function getSemitonesFromC(key) {
