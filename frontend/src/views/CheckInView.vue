        ...
        <template v-else>
            <div class="max-w-4xl mx-auto">
                <div class="flex items-center gap-4 mb-6">
                    <button
                        @click="$router.back()"
                        class="text-gray-600 dark:text-gray-300 dark:hover:text-gray-200 cursor-pointer"
                    >
                        {{ $t("checkin.back") }}
                    </button>
                ...
                <div v-else>
                    ...
                    >
                        <div class="flex items-center justify-between mb-4">
                            <div>
                                ...
                                <p class="text-gray-500 dark:text-gray-400">
                                    {{ currentPlan.service_type_name }}
                                    <span v-if="currentPlan.time">{{
                                        currentPlan.time?.slice(0, 5)
                                    }}</span>
                                </p>
                            </div>
                            <button
                                @click="currentPlan = null"
                                class="text-sm text-gray-500 dark:text-gray-400 dark:hover:text-gray-300 cursor-pointer"
                            >
                                {{ $t("checkin.change") }}
                            </button>
                            <button
                        ...
                        <div v-if="showQR" class="mt-3 text-center">
                            <img
                                :src="`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(origin + '/checkin?plan=' + currentPlan.id)}`"
                ...
                >
                    ...
                    >
                        ...
                        <form
                            @submit.prevent="handleManualCheckIn"
                            class="space-y-4"
                        >
                            <div>
                                <label
                            ...
                            <div class="flex gap-3 justify-end pt-2">
                                <button
                                    type="button"
                                    @click="showManualCheckIn = false"
                                    class="px-4 py-2 text-gray-700 dark:text-gray-200 dark:hover:bg-gray-700 rounded-lg cursor-pointer"
                                >
                                    {{ $t("checkin.cancel") }}
                                </button>
                                <button
...
const formatTime = (tl?: string) =>
    tl
        ? new Date(tl).toLocaleTimeString("fr-FR", {
              hour: "2-digit",
              minute: "2-digit",
          })
        : "";
...
const handleManualCheckIn = async () => {
    ...
    try {
        let member = members.value.find(
            (m) =>
                m.last_name.toLowerCase() ===
                    manualForm.value.last_name.toLowerCase() &&
                (!manualForm.value.first_name ||
                    m.first_name?.toLowerCase() ===
                        manualForm.value.first_name.toLowerCase()),
        );
        if (!member) {
