<template>
    <div class="mx-auto max-w-7xl p-4 pb-24 md:pb-6">
        ...
        <div
            ...
            aria-busy="true"
        >
            <div class="h-8 w-80 rounded bg-gray-200 dark:bg-gray-700"></div>
            <div class="h-5 w-72 rounded bg-gray-100 dark:bg-gray-800"></div>
            <div
                class="h-16 w-[340px] rounded bg-gray-100 dark:bg-gray-800"
            ></div>
            <span class="mt-4 text-gray-400 dark:text-gray-500">{{
                $t("loading")
            }}</span>
        </div>
        ...
        >
            ...
            >
                ...
                <div v-else class="overflow-x-auto">
                    <table class="min-w-full">
                        ...
                        >
                            <tr>
                                <th
                                    class="px-4 py-3 text-left text-sm font-semibold text-gray-600 dark:text-gray-300"
                                >
                                    Title
                                </th>
                                <th
                ...
                <div
                    v-if="filteredSongs.length > pageSize"
...
const filteredSongs = computed(() => {
    ...
    return songs.value.filter((song) => {
        ...
        if (
            ...
            !selectedTags.value.some((tag) => songThemes.includes(tag))
        ) {
            return false;
        }
        if (
            selectedThemes.value.length > 0 &&
            !selectedThemes.value.some((theme) => songThemes.includes(theme))
        ) {
            return false;
        }
        ...
        if (
            ...
            (!song.primary_key ||
                !selectedKeys.value.includes(song.primary_key))
        ) {
            return false;
        }
        ...
        if (selectedPermissions.value.length > 0) {
            ...
            if (!selectedPermissions.value.some((perm) => role.includes(perm)))
                return false;
        }

        const lastUsed = song.last_used
            ? new Date(`${song.last_used}T12:00:00`)
            : null;
        if (
            dateFilter.value === "recent" &&
            (!lastUsed || lastUsed < ninetyDaysAgo)
        )
            return false;
        if (
            dateFilter.value === "year" &&
            (!lastUsed || lastUsed.getFullYear() !== now.getFullYear())
        )
            return false;
        if (
            dateFilter.value === "stale" &&
            (!lastUsed || lastUsed.getFullYear() >= now.getFullYear())
        )
            return false;
...
const totalPages = computed(() =>
    Math.max(1, Math.ceil(filteredSongs.value.length / pageSize)),
);
...
function toggleTeam(teamId: number) {
    if (selectedTeamIds.value.includes(teamId)) {
        selectedTeamIds.value = selectedTeamIds.value.filter(
            (id) => id !== teamId,
        );
    } else {
...
async function loadSongs() {
    ...
    error.value = "";
    try {
        const [songData, teamData] = await Promise.all([
            api.getSongs(),
            api.getTeams(),
        ]);
