<template>
  <div>
    ...
    <div class="flex gap-2 mb-4">
      <button @click="tab = 'configs'" class="px-3 py-1.5 rounded-lg text-sm" :class="tab === 'configs' ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700'">Configuration</button>
      <button @click="tab = 'logs'" class="px-3 py-1.5 rounded-lg text-sm" :class="tab === 'logs' ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700'">Logs</button>
    </div>

    <!-- Configs tab -->
    <div v-if="tab === 'configs'">
      <div v-if="loading" class="text-center py-12 text-gray-500">Chargement...</div>
      <div v-else-if="webhooks.length === 0" class="text-center py-12 text-gray-400">Aucun webhook configuré.</div>
        ...
        <div v-for="wh in webhooks" :key="wh.id"
          ...
          <div class="flex items-start justify-between">
            <div class="flex-1">
              ...
              <div class="text-xs text-gray-400 mt-1">Événements: {{ wh.events }}</div>
            </div>
            <button @click="deleteWebhook(wh.id)" class="text-red-400 hover:text-red-600 text-sm cursor-pointer">Suppr.</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- Logs tab -->
    <div v-if="tab === 'logs'">
      <div v-if="logsLoading" class="text-center py-12 text-gray-500">Chargement...</div>
      <div v-else class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div class="overflow-x-auto">
        <table class="w-full text-sm">
          <thead class="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-600">
            <tr>
              <th class="text-left px-4 py-2 text-gray-600">Date</th>
              <th class="text-left px-4 py-2 text-gray-600">Événement</th>
              <th class="text-left px-4 py-2 text-gray-600">Statut</th>
              <th class="text-left px-4 py-2 text-gray-600">Réponse</th>
            </tr>
          </thead>
    ...
    <div v-if="showForm" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" @click.self="showForm = false">
      <div class="bg-white dark:bg-gray-800 rounded-xl p-6 w-full max-w-lg shadow-xl">
        <h3 class="text-lg font-bold mb-4 text-gray-800 dark:text-gray-100">Nouveau webhook</h3>
        <form @submit.prevent="create" class="space-y-4">
          ...
          <div>
            <label class="block text-sm font-medium mb-1">Événements (séparés par des virgules)</label>
            <input v-model="form.events" placeholder="plan.created, schedule.created, member.created" class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200" />
            <p class="text-xs text-gray-400 mt-1">Utilisez * pour tous les événements</p>
          </div>
          <div>
          ...
          <div>
            <label class="block text-sm font-medium mb-1">Secret (optionnel)</label>
          ...
          <div class="flex gap-3 justify-end pt-2">
            ...
            <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 cursor-pointer">Créer</button>
          </div>
        </form>
      </div>
    </div>
