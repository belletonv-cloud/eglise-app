    ...
    <div class="mx-auto max-w-7xl p-4 pb-24 md:pb-6">
        <div class="mb-4 flex items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-2">
                <div
                    class="flex overflow-hidden rounded-lg border border-gray-200 dark:border-gray-600"
                >
                    <button
                        @click="viewMode = 'list'"
                        :class="[
        ...
        <div
            v-else-if="error"
            class="bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 p-4 rounded-lg"
        >
            {{ error }}
        </div>
        ...
        <div v-else>
            ...
            <div v-if="viewMode === 'list'">
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-sm">
                            <thead class="bg-gray-50 dark:bg-gray-900/40">
                                <tr>
                                    <th
                                        class="px-4 py-2 text-left font-semibold text-gray-500 dark:text-gray-400 dark:text-gray-500"
                                    >
                                        Date
                                    </th>
                                    <th
                            ...
                            <tbody>
                                ...
                                >
                                    ...
                                    <td class="px-4 py-2">
                                        <span
                                            :class="`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${statusClass(plan.status)}`"
                                        >
                ...
                <div v-if="visiblePastPlans.length > 0">
                    <div class="flex items-center gap-3 mb-3">
                        <h3
                            class="text-sm font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500 dark:text-gray-400 dark:text-gray-500"
                        >
                            Past
                        </h3>
                        <button
                            @click="showPast = !showPast"
                            class="text-xs text-blue-500 dark:text-blue-400 hover:underline cursor-pointer"
                        >
                            {{
                                showPast
                                    ? "Masquer"
                                    : `Afficher (${visiblePastPlans.length})`
                            }}
                        </button>
                    </div>
                    <div
            ...
            <div v-else-if="viewMode === 'matrix'">
                    ...
                    <table class="w-full border-collapse">
                            ...
                            >
                                ...
                                >
                                    ...
                                    <div class="text-gray-400 dark:text-gray-500">
                                        {{ formatMonth(plan.date) }}
                                    </div>
                                    <div
                                        v-if="plan.theme"
                                        class="text-xs italic text-indigo-500 truncate max-w-[90px]"
                                        :title="plan.theme"
                                    >
                                        {{ plan.theme }}
                                    </div>
                        ...
                        <tbody>
                            ...
                            >
                                ...
                                >
                                    <div
                                        v-if="planTypeId(plan) === row.type_id"
                                        @click="goToPlan(plan.id)"
                                        class="cursor-pointer inline-flex flex-col items-center gap-1"
                                    >
                                        <span
                                            :class="statusDot(plan.status)"
                                        ></span>
                                        <span
                                            class="text-xs text-gray-600 dark:text-gray-400 dark:text-gray-500"
                                            >{{ plan.items_count }}♪</span
                                        >
                                        <span class="text-xs text-gray-400 dark:text-gray-500"
                                            >{{ plan.people_count }}👤</span
                                        >
                                    </div>
                                    <span
                                        v-else
                                        class="text-gray-200 dark:text-gray-700 text-xs"
                                        >—</span
                                    >
                                </td>
                            </tr>
                ...
                <p class="text-xs text-gray-400 mt-3 text-center">
                    Cliquer sur un service pour voir le détail · ⊞ = 8 prochains
                    services
                </p>
            </div>
            ...
            <!-- Pagination -->
            <div
                v-if="viewMode === 'list' && total > pageSize"
                class="flex items-center gap-2 justify-center py-6"
            >
                <button
                    @click="goPrev"
                    :disabled="page === 1"
                    class="px-3 py-1.5 rounded bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 disabled:opacity-40 text-sm cursor-pointer"
                >
                    ← Préc.
                </button>
            ...
            >
                ...
                <form @submit.prevent="createPlan" class="space-y-4">
                    ...
                    <div>
                        <label
                            class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
                            >{{ $t("plansList.theme") }}</label
                        >
                        <input
                            v-model="form.theme"
                            type="text"
                            :placeholder="$t('plansList.theme')"
                            class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 dark:bg-gray-700 dark:text-gray-100"
                        />
                    </div>
                    <div>
                        <label
                            class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
                            >{{ $t("plansList.notes") }}</label
                        >
                        <textarea
                            v-model="form.notes"
                            rows="2"
                            :placeholder="$t('plansList.notes')"
                            class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 dark:bg-gray-700 dark:text-gray-100"
                        ></textarea>
                    </div>
                ...
                <form @submit.prevent="applyTemplate" class="space-y-4">
                    <div>
                        ...
                        >
                            ...
                            <option
                                v-for="tpl in planTemplates"
                                :key="tpl.id"
                                :value="tpl.id"
                            >
                                {{ tpl.name }} ({{ tpl.item_count }}
                                {{ $t("plansList.items") }})
                            </option>
                        </select>
                    </div>
...
const filteredPlans = computed(() =>
    plans.value.filter(
        (p) =>
            ...
            String(p.service_type_id ?? "") === selectedServiceType.value,
    ),
);
...
const matrixRows = computed(() => {
    ...
    for (const plan of matrixPlans.value) {
        ...
        if (!types.has(key))
            types.set(key, {
                type_id: plan.service_type_id ?? null,
                name: plan.service_type_name || "Sans type",
            });
    }
...
const formatDay = (d: string) =>
    d ? new Date(d + "T12:00:00").getDate().toString().padStart(2, "0") : "";
...
const statusDot = (s: string) =>
    `inline-block w-3 h-3 rounded-full ${s === "completed" ? "bg-green-400" : s === "cancelled" ? "bg-red-400" : "bg-blue-400"}`;

const goToPlan = (id: number) =>
    router.push({ name: "plan-detail", params: { id: id.toString() } });
