<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Sondages</h1>
    ...
    <div v-else-if="polls.length === 0" class="text-center py-12 text-gray-400">Aucun sondage pour le moment.</div>
      ...
      <div v-for="poll in polls" :key="poll.id"
        ...
        <div class="flex items-start justify-between mb-3">
          <div>
            ...
            <p class="text-xs text-gray-400">{{ poll.vote_count }} vote(s) · {{ poll.max_votes }} vote(s) max</p>
          </div>
          <span v-if="poll.expires_at && poll.expires_at < now" class="text-xs bg-gray-100 text-gray-500 px-2 py-1 rounded">Expiré</span>
        </div>
        ...
        <div class="space-y-2">
          <div v-for="opt in poll.options" :key="opt.id"
            ...
            <div class="flex items-center justify-between relative z-10">
              ...
              <span class="text-xs text-gray-400">{{ getVoteCount(poll, opt.id) }} voix</span>
            </div>
    ...
    <div v-if="showCreate" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" @click.self="showCreate = false">
      <div class="bg-white dark:bg-gray-800 rounded-xl p-6 w-full max-w-lg shadow-xl">
        <h3 class="text-lg font-bold mb-4 text-gray-800 dark:text-gray-100">Nouveau sondage</h3>
        <form @submit.prevent="createPoll" class="space-y-4">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Votes max par personne</label>
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Options (une par ligne)</label>
            <textarea v-model="form.optionsText" rows="4" placeholder="Option 1&#10;Option 2&#10;Option 3" class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200"></textarea>
          </div>
          <div class="flex gap-3 justify-end pt-2">
            <button type="button" @click="showCreate = false" class="px-4 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg cursor-pointer">Annuler</button>
...
const vote = async (poll: any, opt: any) => {
  if (poll.expires_at && poll.expires_at < new Date().toISOString()) {
    showToast('Sondage expiré', 'error')
    return
  }
