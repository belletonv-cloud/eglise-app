<template>
  <div class="max-w-5xl mx-auto">
    <h1 class="text-2xl font-bold text-gray-800 mb-2">{{ $t('adminContent.title') }}</h1>
    <p class="text-gray-500 mb-6">{{ $t('adminContent.description') }}</p>

    <div class="mb-4 flex items-center gap-3">
      <label class="text-sm font-medium text-gray-700">{{ $t('adminContent.filter') }} :</label>
      <select v-model="selectedCategory" class="border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white">
      ...
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="key in filteredKeys" :key="key"
            :class="{ 'bg-amber-50': hasOverride(key) }">
            <td class="px-4 py-3 font-mono text-xs text-gray-500 align-top whitespace-nowrap">{{ key }}</td>
            <td class="px-4 py-3 text-gray-800 align-top">{{ currentValue(key) }}</td>
            <td class="px-4 py-3 align-top">
              <textarea
                :value="getOverride(key)"
                @input="setOverride(key, ($event.target as HTMLTextAreaElement).value)"
                rows="2"
                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm resize-y"
                :class="{ 'border-amber-400 ring-1 ring-amber-200': hasOverride(key) }"
              ></textarea>
            </td>
    ...
    <div v-if="savedMessage" class="mt-4 text-sm text-green-600 font-medium">
...
const categories = computed(() => {
  ...
  for (const key of Object.keys(frMessages)) {
    if (typeof frMessages[key] === 'object' && !Array.isArray(frMessages[key]) && frMessages[key] !== null) {
...
const filteredKeys = computed(() => {
  ...
  return allKeys.value.filter(k => k.startsWith(selectedCategory.value + '.'))
})
