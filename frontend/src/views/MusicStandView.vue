<template>
  <div class="music-stand" :class="{ 'stage-mode': stageMode }" @click="toggleToolbar" @keydown="onKeydown" tabindex="0">
    ...
    <div v-if="showToolbar" class="toolbar" @click.stop>
      <button @click="goBack" class="toolbar-btn" :title="$t('musicStand.back')">←</button>

      <div class="toolbar-center">
        <span class="song-title" @click="openSongBrowser">{{ song?.title }}</span>
        <span v-if="arrangement" class="key-badge" @click.stop="showKeyPicker = !showKeyPicker">{{ currentKey }}</span>
      </div>

      <div class="toolbar-right">
        <button v-if="prevSongId" @click.stop="goToPrev" class="toolbar-btn" title="Chant précédent">⏮</button>
        <button v-if="nextSongId" @click.stop="goToNext" class="toolbar-btn" title="Chant suivant">⏭</button>
        <button @click.stop="transpose(-1)" class="toolbar-btn sm:px-1 sm:text-sm" title="-1 demi-ton">−</button>
        <button @click.stop="transpose(1)" class="toolbar-btn sm:px-1 sm:text-sm" title="+1 demi-ton">+</button>
        <button v-if="setlistSongs.length > 0" @click.stop="showSetlist = !showSetlist" class="toolbar-btn" :class="{ active: showSetlist }" :title="$t('setlist.back_to_plan')">☰</button>
        <button @click.stop="toggleMetronome" class="toolbar-btn" :class="{ active: metronomePlaying }" title="Métronome">♩</button>
        <button @click.stop="toggleAutoScroll" class="toolbar-btn" :class="{ active: autoScrollActive }" :title="autoScrollActive ? 'Auto-scroll ON' : 'Auto-scroll OFF'">⟳</button>
        <button @click.stop="toggleStageMode" class="toolbar-btn" :class="{ active: stageMode }" title="Mode scène">🌙</button>
        <button @click.stop="showSettings = !showSettings" class="toolbar-btn" :title="$t('pdfExport.title')">⚙</button>
      </div>
    </div>

    <!-- Song Browser -->
    <div v-if="showSongBrowser" class="song-browser-overlay" @click="showSongBrowser = false">
      <div class="song-browser" @click.stop>
        <div class="browser-header">
          ...
          <button @click="showSongBrowser = false" class="browser-close">✕</button>
        </div>
    ...
    <div v-if="showKeyPicker" class="key-picker" @click.stop>
      <button v-for="k in keyOptions" :key="k" @click="setKey(k)" class="key-option" :class="{ active: k === currentKey }">{{ k }}</button>
    </div>
    ...
    <div v-if="showSettings" class="settings-panel" @click.stop>
      <h4 class="text-sm font-bold mb-2 text-gray-300">{{ $t('pdfExport.title') }}</h4>
      <label class="flex items-center gap-2 text-sm text-gray-300 mb-2">
        <input type="checkbox" v-model="showChords" class="rounded" /> Accords
      </label>
      <label class="flex items-center gap-2 text-sm text-gray-300 mb-2">
        <input type="checkbox" v-model="showLyrics" class="rounded" /> Paroles
      </label>
      <label class="flex items-center gap-2 text-sm text-gray-300 mb-2">
        <input type="checkbox" v-model="showSections" class="rounded" /> Sections
      </label>
      <div class="text-sm text-gray-300 mb-2">
        <span>Taille du texte</span>
        <input type="range" v-model.number="fontSize" min="14" max="48" class="w-full mt-1" />
    ...
    <div v-if="prevSongId || nextSongId" class="song-nav-bottom" @click.stop>
      ...
      <div v-else></div>
      <button v-if="nextSongId" @click="goToNext" class="song-nav-btn">{{ nextSongTitle }} →</button>
    </div>
  </div>
</template>
...
function onKeydown(e: KeyboardEvent) {
  ...
  if (e.key === 'ArrowUp') { transpose(1); e.preventDefault() }
  if (e.key === 'ArrowDown') { transpose(-1); e.preventDefault() }
}
...
async function loadSongData(songId: number, arrId: number | null) {
  ...
  try {
    ...
    if (arrId) {
      ...
      arrangement.value = song.value.arrangements?.find((a: any) => a.chord_chart) || song.value.arrangements?.[0]
    }
